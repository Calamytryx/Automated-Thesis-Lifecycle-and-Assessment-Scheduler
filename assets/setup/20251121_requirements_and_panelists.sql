...
-- ============================================================================
-- 1. ALTER requirements TABLE: Add dynamic defense type and multi-submission support
-- ============================================================================
ALTER TABLE `requirements` ADD COLUMN `requirement_type` ENUM('title_proposal', 'title_defense', 'final_defense', 'general') DEFAULT 'general' COMMENT 'Defense type this requirement applies to' AFTER `name`;
ALTER TABLE `requirements` ADD COLUMN `allow_multiple_submissions` TINYINT(1) DEFAULT 0 COMMENT 'Whether teams can submit multiple files (max 3) for this requirement' AFTER `requirement_type`;
ALTER TABLE `requirements` ADD COLUMN `max_submissions` INT DEFAULT 1 COMMENT 'Maximum number of submissions allowed (default 1, max 3)' AFTER `allow_multiple_submissions`;
...
-- ============================================================================
-- 4. ALTER defense_schedules TABLE: Add defense type tracking and multi-file support
-- ============================================================================
ALTER TABLE `defense_schedules` ADD COLUMN `defense_type` ENUM('title_proposal', 'title_defense', 'final_defense') COMMENT 'Type of defense being scheduled' AFTER `room`;
...
ALTER TABLE `defense_schedules` ADD COLUMN `admin_override_defense_type` TINYINT(1) DEFAULT 0 COMMENT 'Whether defense type was manually overridden by admin' AFTER `related_requirement_files`;
...
-- ============================================================================
-- UPDATE existing requirements with sensible defaults
UPDATE `requirements` SET `requirement_type` = 'title_proposal' WHERE `name` LIKE '%title%' OR `name` LIKE '%proposal%' LIMIT 1;
UPDATE `requirements` SET `requirement_type` = 'final_defense' WHERE `name` LIKE '%manuscript%' OR `name` LIKE '%final%' LIMIT 1;
UPDATE `requirements` SET `requirement_type` = 'title_defense' WHERE `name` LIKE '%capstone%' LIMIT 1;

-- Enable multiple submissions for title proposal requirement (assuming first title-related req)
UPDATE `requirements` SET `allow_multiple_submissions` = 1, `max_submissions` = 3 WHERE `requirement_type` = 'title_proposal' LIMIT 1;
