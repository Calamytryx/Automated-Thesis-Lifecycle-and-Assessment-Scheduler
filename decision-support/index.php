...
// Include the function to get embedded forms
require_once 'get_embedded_form.php';
...
// Get embedded form if available
$embeddedForm = null;
if (isset($schedule['id'])) {
    $embeddedForm = getEmbeddedForm($schedule['id']);
}
          ...
          <?php if ($embeddedForm): ?>
            ...
            <div class="card mb-4 box-shadow h-100">
              <div class="panel-header">
                ...
                <button class="btn btn-outline-secondary" id="toggleDefaultForm">
                  <i class="fas fa-exchange-alt me-2"></i>Toggle Default Form
                </button>
              </div>
              <div class="panel-content" id="embeddedFormContainer">
                <div class="embed-responsive">
                  <?php echo $embeddedForm; ?>
                </div>
              </div>
              
              <!-- Default form (hidden initially) -->
              <div class="panel-content" id="defaultFormContainer" style="display: none;">
                <!-- Original score sheet content goes here -->
                <!-- Toggle Buttons -->
                <div class="toggle-container">
            ...
            <script>
              ...
              document.addEventListener('DOMContentLoaded', function() {
                const toggleBtn = document.getElementById('toggleDefaultForm');
                const embeddedContainer = document.getElementById('embeddedFormContainer');
                const defaultContainer = document.getElementById('defaultFormContainer');
                
                if (toggleBtn) {
                  toggleBtn.addEventListener('click', function() {
                    if (embeddedContainer.style.display !== 'none') {
                      embeddedContainer.style.display = 'none';
                      defaultContainer.style.display = 'block';
                      toggleBtn.innerHTML = '<i class="fas fa-exchange-alt me-2"></i>View Google Form';
                    } else {
                      embeddedContainer.style.display = 'block';
                      defaultContainer.style.display = 'none';
                      toggleBtn.innerHTML = '<i class="fas fa-exchange-alt me-2"></i>View Default Form';
                    }
                  });
                }
              });
...
<script>
  ...
  if (scoreSheetTab) {
    scoreSheetTab.addEventListener('click', function() {
      // Get the defense schedule ID
      const defenseScheduleId = <?php echo $schedule['id']; ?>;
      
      // Fetch the form embed link
      fetch(`get_form_embed.php?defense_schedule_id=${defenseScheduleId}`)
          .then(response => response.json())
          .then(data => {
              if (data.error) {
                  console.log('No custom form available, using default scoring system');
                  return;
              }
              
              // Replace the score sheet content with the embedded form
              const scoreSheetContent = document.getElementById('score-sheet-content');
              if (scoreSheetContent) {
                  // Save the original content
                  if (!scoreSheetContent.getAttribute('data-original-content')) {
                      scoreSheetContent.setAttribute('data-original-content', scoreSheetContent.innerHTML);
                  }
                  
                  // Replace with the embedded form
                      ...
                      <div class="album">
                          <div class="container">
                              <div class="card mb-4 box-shadow h-100">
                                  <div class="panel-header">
                                      <h4>Evaluation Form</h4>
                                      <button class="btn btn-outline-secondary" id="restoreDefaultForm">
                                          <i class="fas fa-undo me-2"></i>Use Default Form
                                      </button>
                                  </div>
                                  <div class="panel-content">
                                      <div class="embed-responsive">
                  ...
                  const restoreButton = document.getElementById('restoreDefaultForm');
                  if (restoreButton) {
                      restoreButton.addEventListener('click', function() {
                          scoreSheetContent.innerHTML = scoreSheetContent.getAttribute('data-original-content');
                      });
                  }
              }
          })
